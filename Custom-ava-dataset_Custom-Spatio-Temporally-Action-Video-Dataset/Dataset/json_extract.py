...
def get_sequential_action_id(option_value):
    """Convert option value to sequential action ID"""
    global global_action_mapping, next_action_id
    option_key = str(option_value)

    if option_key not in global_action_mapping:
        global_action_mapping[option_key] = next_action_id
        next_action_id += 1
        print(f"  Mapped option '{option_value}' to action_id {global_action_mapping[option_key]}")

    return global_action_mapping[option_key]
    ...
    for metadata_key in via_json.get('metadata', {}):
        ...
        for attribute_id in imagen_x.get('av', {}):
            ...
            for av in av_arr:
                ...
                x2 = max(0, min(1, x2))
                y2 = max(0, min(1, y2))
                action_id = get_sequential_action_id(av)
                csv_row = [video_name, frame_id, x1, y1, x2, y2, action_id]
                dicts.append(csv_row)

                print(f"  Added: {video_name} frame {frame_id}, action {action_id}")
...
def main():
    ...
    try:
        ...
        print(f"\nOriginal value to sequential action_id mapping:")
        for original_value, sequential_id in sorted(global_action_mapping.items(), key=lambda x: x[1]):
            print(f"  '{original_value}' -> {sequential_id}")
        mapping_file = './action_id_mapping.json'
        with open(mapping_file, 'w') as f:
            json.dump(global_action_mapping, f, indent=2)
        print(f"\nAction ID mapping saved to {mapping_file}")
