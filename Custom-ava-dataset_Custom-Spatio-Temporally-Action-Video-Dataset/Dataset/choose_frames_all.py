...
if not os.path.exists('./choose_frames_all'):
    os.makedirs('./choose_frames_all')
...
for filepath, dirnames, filenames in os.walk('./frames'):
    ...
    for filename in filenames:
        ...
        # Parse the frame number from filename
        try:
            # Expected format: 1_000001.jpg, 2_000031.jpg, etc.
        ...
        if temp_num in num_frames:
            ...
            # Create destination filename with proper formatting
            temp_num_str = str(temp_num).zfill(6)
            dst_filename = temp_name + "_" + temp_num_str + ".jpg"
            dstpath = os.path.join('./choose_frames_all', dst_filename)
